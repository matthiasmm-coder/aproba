  ...
  return (
    <div className="mb-6 rounded-2xl border border-aproba-200 bg-aproba-50/60 p-5">
      <div className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-sm font-bold text-slate-900">Termina de configurar tu despacho</h2>
          <p className="mt-0.5 text-xs text-slate-500">{hechos} de {items.length} completado{hechos === 1 ? "" : "s"}. Te llevará un par de minutos.</p>
        </div>
        <button onClick={() => { localStorage.setItem(KEY, "1"); setDismissed(true); }} aria-label="Ocultar" className="rounded-md p-1 text-slate-400 transition hover:bg-white hover:text-slate-600">
