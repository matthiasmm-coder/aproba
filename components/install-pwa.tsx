...
export function InstallPWA() {
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-cream-50/60 p-5">
      ...
      <div className="mt-4">
        {available || ios ? cta : (
          <p className="text-xs text-slate-400">{t("Abre Aproba en Chrome, Edge o Safari y usa la opción «Instalar» / «Añadir a la pantalla de inicio» del navegador.")}</p>
        )}
        {iosHint && (
          <p className="mt-3 flex items-start gap-2 rounded-lg bg-white px-3 py-2 text-xs leading-relaxed text-slate-600">
            ...
            {t("En iPhone/iPad: pulsa Compartir y luego «Añadir a la pantalla de inicio».")}
          </p>
        )}
      </div>
    </div>
