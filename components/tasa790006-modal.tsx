...
// Tasa 790-006 (certificado de ANTECEDENTES PENALES, Ministerio de Justicia).
// Misma factura que el modal de la 790-026 y sin captcha: la Sede sirve el impreso
// oficial directamente (justificante único por descarga). Aquí solo se revisan los
// datos. El impreso lleva dos bloques con la misma persona (quien solicita y quien
// recibe el certificado): los rellenamos los dos con estos campos.
...
const CAMPOS: { k: string; label: string; w: string; req?: boolean; tel?: boolean }[] = [
  { k: "numId", label: "NIE, DNI o pasaporte", w: "half", req: true },
...
// Bloque B del impreso: datos de la persona sobre la que se pide el certificado.
const CAMPOS_B: { k: string; label: string; w: string; req?: boolean }[] = [
  { k: "fechaNac", label: "Nacimiento (dd/mm/aaaa)", w: "third", req: true },
  { k: "poblacionNac", label: "Población de nacimiento", w: "third" },
  { k: "provinciaNac", label: "Provincia o país de nacimiento", w: "third" },
  { k: "nacionalidad", label: "País de nacionalidad", w: "third" },
  { k: "nombrePadre", label: "Nombre del padre", w: "third" },
  { k: "nombreMadre", label: "Nombre de la madre", w: "third" },
  { k: "finalidad", label: "Finalidad para la que se solicita", w: "half" },
  { k: "paisDestino", label: "Si va al extranjero: país de destino", w: "third" },
  { k: "autoridadDestino", label: "Autoridad ante la que surte efectos", w: "third" },
];

const CAMPOS_FIN: { k: string; label: string; w: string; req?: boolean }[] = [
...
// clienteId (expediente familiar): la tasa es NOMINATIVA, una por persona.
// expedienteId opcional: desde la ficha del cliente se genera sin archivar.
export function Tasa790006Modal({ expedienteId, clienteId, etiqueta }: { expedienteId?: string; clienteId?: string; etiqueta?: string }) {
  const t = useT();
  ...
  const [correoPostal, setCorreoPostal] = useState(false);
  ...
  async function iniciar() {
    ...
    const r = await fetch("/api/tasa790006/iniciar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ expedienteId, clienteId }) });
  ...
  const TODOS = [...CAMPOS, ...CAMPOS_B, ...CAMPOS_FIN];
  const faltan = TODOS.filter((f) => f.req && !(campos[f.k] ?? "").trim()).map((f) => t(f.label));
  ...
  async function descargar() {
    ...
    const body = { expedienteId, clienteId, campos: { ...campos, correoPostal } };
    const r = await fetch("/api/tasa790006/descargar", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (r.headers.get("content-type")?.includes("pdf")) {
      const blob = await r.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a"); a.href = url; a.download = `tasa-790-006-${expedienteId ?? clienteId ?? "cliente"}.pdf`; a.click();
  ...
  const campo = (f: { k: string; label: string; w: string; req?: boolean; tel?: boolean }) => (
  ...
  return (
    <>
      <button onClick={iniciar} className="inline-flex items-center gap-2 rounded-lg bg-slate-800 px-3.5 py-2 text-sm font-semibold text-white transition hover:bg-slate-900">
        ...
        {etiqueta ?? t("Tasa 790-006 (antecedentes penales)")}
      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/50 backdrop-blur-sm sm:p-4" onClick={() => !enviando && setOpen(false)}>
          <div className="mt-4 w-full max-w-2xl rounded-t-2xl border border-slate-200 bg-white p-4 pb-[max(1rem,env(safe-area-inset-bottom))] shadow-xl sm:mt-6 sm:rounded-2xl sm:p-6" onClick={(e) => e.stopPropagation()}>
            <div className="mb-1 flex items-start justify-between">
              <h2 className="text-lg font-bold text-slate-900">{t("Tasa 790-006 · Antecedentes penales")}</h2>
            ...
            <p className="mb-4 text-xs text-slate-500">{t("Descargamos el impreso oficial de la Sede del Ministerio de Justicia (con su número de justificante) y lo rellenamos con estos datos. Se marca la casilla «Antecedentes penales».")}</p>

            {cargando && <p className="py-10 text-center text-sm text-slate-500">{t("Cargando los datos del solicitante…")}</p>}

            {listo && !cargando && (
              <>
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-6">{CAMPOS.map(campo)}</div>

                <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3">
                  <p className="mb-2 text-[11px] font-medium uppercase tracking-wide text-slate-400">{t("Persona del certificado (apartado B del impreso)")}</p>
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-6">{CAMPOS_B.map(campo)}</div>
                </div>

                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-6">{CAMPOS_FIN.map(campo)}</div>

                <label className="mt-3 flex items-center gap-2 text-sm text-slate-600">
                  <input type="checkbox" className="h-4 w-4 rounded border-slate-300 text-aproba-600" checked={correoPostal} onChange={(e) => setCorreoPostal(e.target.checked)} />
                  {t("Autorizo el envío del certificado por correo postal")}
                </label>
                ...
                <div className="mt-5 flex items-center justify-between gap-2">
                  <a href="https://sede.mjusticia.gob.es/es/tramites/certificado-antecedentes" target="_blank" rel="noreferrer" className="text-xs text-slate-400 underline hover:text-slate-600">{t("Abrir en la Sede oficial")}</a>
