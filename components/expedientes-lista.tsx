...
// Nº oficial editado durante esta visita (id → número). La fila lo enseña aunque su
// carpeta se pliegue y se reabra (las filas del historial vienen de una caché) y la
  ...
  return (
    <div className={`group flex flex-wrap items-center gap-x-3 gap-y-1.5 border-t border-slate-50 py-2.5 pr-3 transition hover:bg-cream-50 ${sangria}`}>
      ...
      {/* Nº de expediente de Extranjería (Jennifer, 24/09): se lee y se corrige en la fila.
          Con casillas va delante de ellas (en el móvil bajan juntos a su línea y el nombre
          no se aprieta); sin casillas (historial), en la línea de la categoría. */}
      {!(!cerrado && comp) && <NumeroOficial key={`${e.id}:${numero}`} expedienteId={e.id} inicial={numero} variante="fila" onGuardado={(n) => numeros.guardar(e.id, n)} className="order-last sm:order-none" />}
      ...
      {!cerrado && comp && (
        <span className="order-last flex basis-full flex-wrap items-center gap-1.5 sm:order-none sm:basis-auto sm:gap-1">
          <NumeroOficial key={`${e.id}:${numero}`} expedienteId={e.id} inicial={numero} variante="fila" onGuardado={(n) => numeros.guardar(e.id, n)} className="sm:mr-1" />
...
export function ExpedientesLista({ items, asignados, temas, packs = [], filtroInicial = null, vistaInicial = "curso", renovaciones = 0, requerimientosTotal = 0, requerimientosFuera = [], archivo = null, avatares = {}, carpetasVacias = [] }: {
  ...
  const ctxNumeros = useMemo(() => ({ editados: numerosEditados, guardar: guardarNumero }), [numerosEditados, guardarNumero]);
  ...
  const pasaTexto = (e: ItemLista) => {
    ...
    return norm(e.clienteNombre).includes(nq) || norm(e.clienteNacionalidad).includes(nq) || norm(grupoDe(e)).includes(nq)
      ...
      || norm(numerosEditados.get(e.id) ?? e.numeroOficial ?? "").includes(nq)
  ...
  return (
    ...
    <NumerosCtx.Provider value={ctxNumeros}>
    <div>
