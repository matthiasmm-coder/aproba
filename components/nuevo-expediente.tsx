...
export function NuevoExpediente() {
  ...
  async function crear() {
    ...
    try {
      ...
      if (!mem) throw new Error("No se encontró tu despacho.");
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      {step === 0 && (
        <div>
          <h1 className="text-2xl font-bold tracking-tightest text-slate-900">Nuevo expediente</h1>
          <p className="mt-1 text-slate-500">Elige el cliente. Le enviarás un enlace y él elegirá su trámite y subirá sus documentos.</p>

          {/* Bascule existant / nouveau */}
          <div className="mt-5 inline-flex gap-1 rounded-lg bg-slate-100 p-1">
            <button onClick={() => { setModoNuevo(false); setError(null); }} className={`rounded-md px-4 py-2 text-sm font-medium transition ${!modoNuevo ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Cliente existente</button>
            <button onClick={() => { setModoNuevo(true); setSeleccionado(null); setError(null); }} className={`rounded-md px-4 py-2 text-sm font-medium transition ${modoNuevo ? "bg-white text-slate-900 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}>Cliente nuevo</button>
          </div>
          ...
          {!modoNuevo ? (
            <div className="mt-4">
              <div className="relative">
                ...
                <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente…" className="w-full rounded-lg border border-slate-300 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-aproba-600 focus:ring-2 focus:ring-aproba-100" />
              </div>
              <div className="mt-3 max-h-72 space-y-1.5 overflow-y-auto pr-1">
                ...
                {filtrados.length === 0 && <p className="rounded-xl border border-dashed border-slate-200 px-4 py-6 text-center text-sm text-slate-400">Sin resultados. ¿Es un <button onClick={() => setModoNuevo(true)} className="font-semibold text-aproba-700 hover:underline">cliente nuevo</button>?</p>}
              </div>
            </div>
          ) : (
      ...
      {step === 1 && (
        <div className="text-center">
          ...
          <h1 className="mt-5 text-2xl font-bold tracking-tightest text-slate-900">Expediente creado</h1>
          ...
          <div className="mt-7 rounded-2xl border border-slate-200 bg-white p-5 text-left">
            <p className="text-sm font-semibold text-slate-800">Enlace para tu cliente</p>
            <p className="mt-1 text-xs text-slate-500">Envíaselo por WhatsApp. Elegirá su trámite y subirá sus datos y documentos sin instalar nada.</p>
          ...
          <div className="mt-5 flex justify-center gap-3 text-sm">
            <Link href="/app/expedientes" className="font-semibold text-aproba-700 hover:underline">Ir al tablero</Link>
