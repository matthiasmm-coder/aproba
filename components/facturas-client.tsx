...
export function FacturasClient({ facturas }: { facturas: Factura[] }) {
  ...
  if (mode === "mtd") {
    ...
    rangeTo = HOY;
    rangeLabel = `${MESES[HOY.getMonth()]} ${HOY.getFullYear()}, hasta hoy`;
  } else if (mode === "ytd") {
    rangeFrom = new Date(HOY.getFullYear(), 0, 1);
    rangeTo = HOY;
    rangeLabel = `Año ${HOY.getFullYear()}, hasta hoy`;
  } else {
  ...
  const STATS = [
    { label: "Facturado", value: eur(facturado), sub: `${filtered.length} facturas`, tone: "text-slate-900" },
    { label: "Cobrado", value: eur(cobrado), sub: "Pagadas", tone: "text-aproba-700" },
    { label: "Pendiente de cobro", value: eur(pendiente), sub: vencidas ? `${vencidas} vencidas` : "Al día", tone: "text-amber-600" },
  ];
  ...
  return (
    <div className="mx-auto max-w-4xl">
      <div className="mb-6 flex items-end justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tightest text-slate-900">Facturas</h1>
          <p className="text-sm text-slate-500">Factura a tus clientes por cada trámite.</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Link href="/app/facturas/nueva" className="rounded-lg bg-aproba-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-aproba-700">+ Nueva factura</Link>
        </div>
      </div>
        ...
        {mode === "custom" && (
          <div className="relative">
            <button onClick={() => setCalOpen((o) => !o)} className="inline-flex items-center gap-2 rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-slate-400">
              ...
              {from && to ? `${fmtFecha(from)} – ${fmtFecha(to)}` : "Elegir fechas"}
            </button>
            {calOpen && (
              <>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-slate-100 text-left text-xs uppercase tracking-wide text-slate-400">
              ...
              <th className="hidden px-5 py-3 font-semibold md:table-cell">Concepto</th>
              ...
              return (
                <tr key={f.id} className="border-b border-slate-50 last:border-0 hover:bg-cream-50">
                  ...
                  <td className="hidden px-5 py-3 text-slate-500 md:table-cell">
                    {f.concepto}
                    {f.origen === "AUTOMATICA" && (
                      <span title="Generada automáticamente al pagar el cliente en la plataforma" className="ml-2 inline-flex items-center gap-1 rounded-full bg-aproba-50 px-1.5 py-0.5 align-middle text-[10px] font-semibold text-aproba-700">
                  ...
                  <td className="px-5 py-3 text-right"><span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${meta.pill}`}>{meta.label}</span></td>
                </tr>
              );
            })}
            {filtered.length === 0 && (
              <tr><td colSpan={6} className="px-5 py-10 text-center text-sm text-slate-400">Sin facturas en este periodo.</td></tr>
            )}
          </tbody>
        </table>
      </div>
