  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const cols = "clave, label, active, precio, anticipo, resto, categoria, porcentaje, oficinaId";
        let res = await sb.from("ServicioConfig").select(cols).order("orden");
        if (res.error) res = await sb.from("ServicioConfig").select("clave, label, active, precio, anticipo, resto").order("orden") as typeof res;
        const filas = (res.data ?? []) as { clave: string; label: string | null; active: boolean | null; precio: number | string | null; anticipo: number | string | null; resto: number | string | null; categoria?: string | null; porcentaje?: number | null; oficinaId?: string | null }[];
        ...
        setServicios(usar
          ...
          .map((s) => ({
            ...
            precio: Number(s.precio) || (Number(s.anticipo) || 0) + (Number(s.resto) || 0),
