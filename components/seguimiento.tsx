...
export type SegDoc = { label: string; status: "ok" | "procesando" | "rechazado" | "pendiente" };

const LANG_KEY = "aproba.portal.lang";
const ORDEN: Record<string, number> = { BORRADOR: 0, DOCS_PENDIENTES: 1, DOCS_VALIDADOS: 2, FORM_GENERADO: 3, PRESENTADO: 4, RESUELTO: 5, RECHAZADO: 1 };
...
export function Seguimiento({
  token, gestoria, clienteNombre, idioma, referencia, estado, docs: docsIniciales,
}: {
  token: string; gestoria: string; clienteNombre: string; idioma: string; referencia: string; estado: string; docs: SegDoc[];
}) {
                ...
                <div key={d.label} className="rounded-xl border border-slate-200 bg-white p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    {(d.status === "pendiente" || d.status === "rechazado") && (
                      <button onClick={() => pedirArchivo(i)} disabled={subiendoEste} className="shrink-0 rounded-lg bg-aproba-600 px-3 py-1.5 text-xs font-semibold text-white transition hover:bg-aproba-700 disabled:bg-slate-300">
