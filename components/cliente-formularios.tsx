...
import { Tasa790006Modal } from "@/components/tasa790006-modal";
import { Tasa790052Modal } from "@/components/tasa790052-modal";
import { Tasa790062Modal } from "@/components/tasa790062-modal";
    ...
    <div className="mt-6 rounded-2xl border border-slate-200 bg-white p-6">
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <select
          ...
          className="min-w-0 flex-1 rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-[16px] sm:text-sm outline-none focus:border-aproba-600 focus:ring-2 focus:ring-aproba-100"
        >
          {formularios.map((f) => (
      ...
      <div className="mt-3 border-t border-slate-100 pt-3 text-center">
        <p className="mb-2 text-xs text-slate-500">
          {t("Las tasas se generan con los datos de este cliente ya rellenados: la 790-012 en la Sede de la Policía Nacional, la 790-052 (autorizaciones de residencia) y la 790-062 (autorizaciones de trabajo) en la de Administraciones Públicas y la 790-026 (nacionalidad) y la 790-006 (antecedentes penales) en la del Ministerio de Justicia.")}
        </p>
        ...
        <div className="flex flex-wrap justify-center gap-2">
          ...
          <Tasa790062Modal clienteId={clienteId} etiqueta={t("Tasa 790-062 (trabajo)")} />
          <Tasa790026Modal clienteId={clienteId} etiqueta={t("Tasa 790-026 (nacionalidad)")} />
          <Tasa790006Modal clienteId={clienteId} etiqueta={t("Tasa 790-006 (antecedentes penales)")} />
        </div>
      </div>
    </div>
  );
