      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-sm" onClick={() => !enviando && setOpen(false)}>
          <div className="mt-6 w-full max-w-2xl rounded-2xl border border-slate-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mb-4 text-xs text-slate-500">{t("Generamos el impreso barcodé en la Sede de la Policía Nacional. Completa los campos obligatorios (en ámbar) y escribe el código de seguridad.")}</p>
            ...
            {datos && !cargando && (
              <>
                ...
                <div className="mt-4 flex flex-wrap items-end gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3">
                  ...
                  <img src={datos.captcha} alt={t("Código de seguridad")} className="h-16 w-auto rounded border border-slate-300 bg-white" style={{ imageRendering: "auto" }} />
                  <button onClick={() => iniciar(true)} title={t("Otro código")} className="mb-1 rounded-md border border-slate-300 bg-white p-2 text-slate-500 hover:text-aproba-700" aria-label={t("Refrescar código")}>
                  ...
                  <div className="min-w-[160px] flex-1">
                    ...
                    <input className="w-full rounded-md border border-slate-300 px-2.5 py-2 text-base tracking-widest outline-none focus:border-aproba-600 focus:ring-2 focus:ring-aproba-100" value={captcha} onChange={(e) => setCaptcha(e.target.value)} placeholder={t("Escribe el código")} autoFocus />
                  </div>
                </div>
