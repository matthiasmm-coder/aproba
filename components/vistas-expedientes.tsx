...
// «Requerimientos» fue una cuarta vista del 21 al 26/09/2026: ahora es un FILTRO de «En
// curso», junto a «Esperando al cliente» (Matthias): el requerimiento se ve en la fila de
// su expediente, dentro de su carpeta. /app/requerimientos redirige al filtro.
export type VistaExpedientes = "curso" | "historial" | "renovaciones";
...
const RUTAS: Record<VistaExpedientes, string> = { curso: "/app/expedientes", historial: "/app/expedientes?vista=historial", renovaciones: "/app/vencimientos" };

// Requerimientos (21/09/2026, petición de Jennifer): un reloj, porque lo que los define es
// el plazo. Lo usan el filtro de «En curso» y la línea del requerimiento en la fila.
export function RequerimientosIcon({ className = "" }: { className?: string }) {
...
export function VistasExpedientes({ activa, totalHistorial = 0, totalRenovaciones = 0, onCambiar }: {
  ...
  return (
    // Si no caben en un móvil, se desplazan dentro de su caja en vez de ensanchar toda la
    // página (24/09/2026).
    <div className="inline-flex max-w-full gap-1 overflow-x-auto rounded-lg bg-slate-100 p-1 [scrollbar-width:none]">
      {(["curso", "historial", "renovaciones"] as const).map((v) =>
        onCambiar && v !== "renovaciones"
