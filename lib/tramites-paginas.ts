...
export function paginaDeTramite(t: Tramite): PaginaPublica {
  ...
  const bloques: Bloque[] = [
    ...
    { t: "esquema", titulo: `Un expediente de ${t.nombre.toLowerCase()}, de principio a fin`, nodos: [
      ...
      { titulo: t.formularios.length ? "Formularios y cobro" : "Tasa y cobro", texto: `${t.formularios.length ? `${t.formularios.map((f) => f.code).join(", ")} y la tasa, rellenados y editables.` : "Tasa generada con los datos del cliente."} Anticipo y pago final desde el expediente (Pro y Business).` },
