...
export type FacturaPago = {
  ...
  metodoPago: "TARJETA" | "TRANSFERENCIA" | "EFECTIVO" | "OTRO" | null;
};
...
function mapearDetalle(data: unknown): ExpedienteDetalle {
  ...
  return {
    ...
    validadoManual: Boolean((e as unknown as { validadoAt?: string | null }).validadoAt),
    ...
    facturasPago: (e.facturas ?? []).map((f) => ({
      ...
      metodoPago: (["TARJETA", "TRANSFERENCIA", "EFECTIVO", "OTRO"].includes(f.metodoPago ?? "") ? f.metodoPago : null) as FacturaPago["metodoPago"],
    })),
  };
}
...
export function progresoDeExpediente(
  e: {
    ...
    // Ficha del cliente: alimenta la parte «Información» de la completitud. Se pasa el
    // objeto cliente crudo y aquí se cuenta contra FICHA_CAMPOS — así el tablero y la
    // ficha usan el MISMO denominador que la sección «Información» de la pantalla.
