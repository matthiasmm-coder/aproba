...
// GET → descarga un documento suelto del cliente (bucket privado → service_role).
// Autorización: la lectura de DocumentoCliente va bajo sesión (RLS) → 404 si no es del workspace.
export async function GET(_req: Request, { params }: { params: Promise<{ id: string; docId: string }> }) {
  ...
  const { data: doc } = await supabase.from("DocumentoCliente").select("storagePath, nombreArchivo, mimeType, tipo").eq("id", docId).eq("clienteId", id).maybeSingle();
...
// DELETE → elimina un documento suelto (metadato + archivo del storage).
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string; docId: string }> }) {
  ...
  const { data: doc } = await supabase.from("DocumentoCliente").select("storagePath").eq("id", docId).eq("clienteId", id).maybeSingle();
  if (!doc) return NextResponse.json({ error: "Documento no encontrado." }, { status: 404 });

  const admin = createSupabaseAdmin();
  if (doc.storagePath) await admin.storage.from("documentos").remove([doc.storagePath as string]).catch(() => {});
  const { error } = await admin.from("DocumentoCliente").delete().eq("id", docId).eq("clienteId", id);
