...
export default function NuevaFactura() {
  ...
  async function crear() {
    ...
    try {
      ...
      if (e1 || !mem) throw new Error(e1?.message ?? "No se encontró tu despacho.");
    ...
    return (
      <div>
        <div className="mx-auto mb-5 max-w-2xl rounded-xl border border-aproba-200 bg-aproba-50 px-4 py-3 text-sm text-aproba-700 print:hidden">
          ✓ Factura <span className="font-mono font-semibold">{factura.numero}</span> creada y guardada.{" "}
          <Link href="/app/facturas" className="font-semibold underline">Ver todas →</Link>
        </div>
  ...
  return (
    <div className="mx-auto max-w-xl">
      ...
      <h1 className="text-2xl font-bold tracking-tightest text-slate-900">Nueva factura</h1>
      <p className="mt-1 text-slate-500">Elige el servicio y la tarifa se rellena sola. El IVA y el total se calculan solos.</p>

      <div className="mt-6 space-y-4">
        <div>
          <label className="text-sm font-medium text-slate-700">Cliente</label>
          <input value={cliente} onChange={(e) => setCliente(e.target.value)} placeholder="Nombre del cliente" className="mt-1.5 w-full rounded-lg border border-slate-300 px-3 py-2.5 text-sm outline-none focus:border-aproba-600 focus:ring-2 focus:ring-aproba-100" />
        </div>
        <div>
          ...
          <select value={concepto} onChange={(e) => elegirConcepto(e.target.value)} className="mt-1.5 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm outline-none focus:border-aproba-600 focus:ring-2 focus:ring-aproba-100">
            ...
            <optgroup label="Otros">
              {GENERICOS.map((c) => <option key={c} value={c}>{c}</option>)}
            </optgroup>
          </select>
        </div>
        <div>
          <label className="flex items-center justify-between text-sm font-medium text-slate-700">
            ...
            {autollenado && <span className="text-xs font-normal text-aproba-700">↩ tarifa del servicio · puedes ajustarla</span>}
          </label>
        ...
        <div className="rounded-xl border border-slate-200 bg-cream-50 p-4">
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between text-slate-500"><span>Base imponible</span><span>{eur(baseNum)}</span></div>
            <div className="flex justify-between text-slate-500"><span>IVA ({Math.round(IVA * 100)} %)</span><span>{eur(ivaDe(baseNum))}</span></div>
            <div className="mt-1 flex justify-between border-t border-slate-200 pt-2 text-base font-bold text-slate-900"><span>Total</span><span>{eur(totalDe(baseNum))}</span></div>
          </div>
        </div>
